(function () {
  ...
  const StorageManager = {
    ...
    save(settings) {
      ...
      window.dispatchEvent(
        new CustomEvent('bio:settings-update', { detail: settings }),
      );
    },
  };
  ...
  class IntentWall {
    static init() {
      ...
      input.addEventListener('input', () => {
        ...
        if (words.length >= 10 && uniqueWords.size >= 5) {
          ...
          submit.onmouseover = () => {
            ...
            submit.style.boxShadow = '6px 6px 0px rgba(0,0,0,1)';
          };
          submit.onmouseout = () => {
            submit.style.transform = 'translate(0,0)';
            submit.style.boxShadow = '4px 4px 0px rgba(0,0,0,1)';
          };
          submit.onmousedown = () => {
            submit.style.transform = 'translate(4px,4px)';
            submit.style.boxShadow = '0px 0px 0px rgba(0,0,0,1)';
          };
          submit.onmouseup = () => {
            submit.style.transform = 'translate(-2px,-2px)';
            submit.style.boxShadow = '6px 6px 0px rgba(0,0,0,1)';
      ...
      overlay.querySelector('#bio-intent-cancel').onclick = () => {
        history.back();
      };
      submit.onclick = () => {
  ...
  class HardBlock {
    static init() {
      ...
      overlay.innerHTML = `
            ...
            `;
      document.documentElement.appendChild(overlay);
      overlay.querySelector('#bio-hardblock-leave').onclick = () => {
        history.back();
        window.close();
      };
    }
  ...
  class BreatheWall {
    static init() {
      ...
      overlay.innerHTML = `
            ...
            `;
      overlay.querySelector('#bio-breathe-text').textContent = lines[0];
  ...
  class SessionMonitor {
    ...
    resume() {
      if (
        (PREFS.igTimeMonitor && ENV.IG) ||
        (ENV.BLOCKED && !ENV.IG && !ENV.YT)
      ) {
        this.start();
      }
    }
    ...
    tick() {
      ...
      if (!this.pill.querySelector('.time-text')) {
        this.pill.innerHTML = '<span>⏳</span> <span class="time-text"></span>';
      }
      this.pill.querySelector('.time-text').textContent =
    ...
    renderPill() {
      ...
      this.pill.onmouseover = () => {
        this.pill.style.transform = 'translate(-2px, -2px)';
        this.pill.style.boxShadow = '6px 6px 0px #000000';
      };
      this.pill.onmouseout = () => {
        this.pill.style.transform = 'translate(0, 0)';
        this.pill.style.boxShadow = '4px 4px 0px #000000';
      };
      this.pill.onmousedown = () => {
        this.pill.style.transform = 'translate(4px, 4px)';
        this.pill.style.boxShadow = '0px 0px 0px #000000';
      };
      this.pill.onmouseup = () => {
        this.pill.style.transform = 'translate(-2px, -2px)';
        this.pill.style.boxShadow = '6px 6px 0px #000000';
    ...
    getAIInsight(seconds) {
      ...
      if (minutes < 1) {
        ...
        const text =
          shortInsights[Math.floor(Math.random() * shortInsights.length)];
      ...
      const reflection =
        reflectivePrompts[Math.floor(Math.random() * reflectivePrompts.length)];
    ...
    showRealityCheck() {
      ...
      card.innerHTML = `
                ...
                <div id="bio-time-wasted-display" style="font-size:60px; font-weight:800; color:#000000; margin:0 0 32px 0; line-height:1; font-variant-numeric: tabular-nums;"></div>
                <div id="bio-ai-insight-container" style="background:#ffffff; border-radius:4px; padding:24px; margin-bottom:32px; border:2px solid #000000; box-shadow: 4px 4px 0px #000000;">
                </div>
            ...
            `;
      card.querySelector('#bio-time-wasted-display').textContent =
        this.formatTime(seconds);
      const insightHtml = this.getAIInsight(seconds)
        .replace(/#E5E5EA|#F5F5F7/g, '#000000')
        .replace(/#A1A1A6/g, '#555555');
      const parser = new DOMParser();
      const doc = parser.parseFromString(insightHtml, 'text/html');
      card
        .querySelector('#bio-ai-insight-container')
      ...
      document.getElementById('bio-insight-close').onclick = () =>
        overlay.remove();
      overlay.onclick = (e) => {
  ...
  const InstagramHandler = {
    ...
    routeCheck() {
      ...
      if (
        PREFS.igHideReels &&
        (path.includes('/reels/') || path === '/reels')
      ) {
        window.location.replace('https://www.instagram.com/');
      }

      if (
        (PREFS.igHideReels || PREFS.igHidePosts) &&
    ...
    cleanseDOM() {
      ...
      if (PREFS.igHideReels || PREFS.igFocusMode) {
        ...
        document.querySelectorAll('article').forEach((art) => {
          if (
            art.querySelector('a[href*="/reels/"]') ||
      ...
      articles.forEach((art) => {
        ...
        if (!isDistraction) {
          ...
          for (const el of textElements) {
            ...
            if (
              t.includes('suggested for you') ||
              t.includes('suggested posts')
            ) {
              isDistraction = true;
              break;
            }
  ...
  const YouTubeHandler = {
    ...
    applyCSS() {
      if (PREFS.ytCleanEndScreen) {
        StyleInjector.add(
          'bio-yt-ends',
          '.ytp-ce-element, .ytp-ce-cover-overlay { display: none !important; }',
        );
      }
    ...
    mitigateAds() {
      ...
      if (isAd) {
        ...
        if (PREFS.ytMuteAds && video.muted && video.volume > 0)
          video.muted = false;
      }
    },
  };
  ...
  chrome.storage.onChanged.addListener((changes, namespace) => {
    if (namespace === 'local' && changes.bio_pro_settings) {
      ...
      window.dispatchEvent(
        new CustomEvent('bio:settings-update', { detail: PREFS }),
      );
    }
  });
